...
(function (global) {
  ...
  var schema = (CPW.schema = {
    ...
    normalizeCondition: function (raw) {
      ...
      list.forEach(function (it) {
        ...
        if (!util.isPlainObject(it)) return;
        var t = util.byId(D.conditions, it.type);
        if (!t) return;                                            // 未知IDは安全に捨てる
        var sev = util.byId(D.conditionSeverities, it.severity) ? it.severity : 'moderate';
        var ext = util.byId(D.conditionExtents, it.extent) ? it.extent : 'localized';
        var seen = {};
        var pls = (Array.isArray(it.placements) ? it.placements : []).filter(function (p) {
          if (seen[p]) return false;
          seen[p] = true;
          return !!util.byId(D.conditionPlacements, p);
        });
        items.push({ type: t.id, group: t.group, severity: sev, extent: ext, placements: pls });
      });
      return { items: items };
    },
    ...
    function draw() {
      ...
      panel.appendChild(selectField({
        ...
        set: function (id, sel) {
          ...
          // 状態・加工の部位候補は大分類に依存する。
          // 人魚へ切り替えた直後も、脚・靴を残さず尾・尾びれへ即時更新する。
          var conditionPanel = document.getElementById('panel-condition');
          if (conditionPanel) buildConditionSection(conditionPanel);
          draw();
        }
      }));
  ...
  /* ---------- 3.5 衣装の状態・加工（Phase 5B 後半） ---------- */
  function buildConditionSection(panel) {
    function items() { return state.outfit.condition.items; }
    function setItems(next) { state.set('condition.items', next); }

    /* 部位の候補。人魚では脚・靴系を出さず、尾・尾びれを出す。 */
    function placementPool() {
      var isMer = state.outfit.garment.category === 'merfolk';
      return D.conditionPlacements.filter(function (p) {
        if (isMer) return !p.legRelated;
        return !p.merfolkOnly;
      });
    }
    ...
    function draw() {
      ...
      panel.appendChild(ui.el('p', {
        class: 'p p--note',
        text: '汚れ・破損・濡れ・使用感などを、素材や装飾とは別の層として指定できます。最大2件までです。何も選ばなければ「状態指定なし」のまま出力されます。'
      }));

      if (!items().length) {
        panel.appendChild(ui.el('p', { class: 'field-note', text: '現在は「状態指定なし」です。' }));
      }

      items().forEach(function (item, idx) {
        var card = ui.el('div', { class: 'deco-item' });
        var def = util.byId(D.conditions, item.type);

        card.appendChild(ui.el('div', { class: 'deco-head' }, [
          ui.el('span', { class: 'deco-index', text: '状態・加工 ' + (idx + 1) + (def ? '（' + def.labelJa + '）' : '') }),
          ui.el('button', {
            type: 'button', class: 'btn btn--sm btn--danger', text: '外す',
            'aria-label': '状態・加工 ' + (idx + 1) + ' を外す',
            'data-fkey': 'cond:' + idx + ':remove',
            onclick: function () {
              setItems(items().filter(function (_, i) { return i !== idx; }));
              draw();
              ui.toast('状態・加工を外しました。');
            }
          })
        ]));
        ...
        var patch = function (key, v) {
          ...
          next[idx][key] = v;
          if (key === 'type') {
            var d = util.byId(D.conditions, v);
            next[idx].group = d ? d.group : 'state';
          }
          setItems(next);
        };
        ...
        // 種類（分類ごとに並べる）
        card.appendChild(ui.el('p', { class: 'field-sub', text: '種類' }));
        D.conditionFamilies.forEach(function (fam) {
          var pool = D.conditions.filter(function (c) { return c.family === fam.id; });
          if (!pool.length) return;
          card.appendChild(ui.el('p', { class: 'chip-group-label', text: fam.labelJa }));
          var row = ui.el('div', { class: 'chips', role: 'group', 'aria-label': fam.labelJa });
          pool.forEach(function (opt) {
            var sel = item.type === opt.id;
            var key = 'cond:' + idx + ':type:' + opt.id;
            var b = ui.el('button', {
              type: 'button', class: 'chip' + (sel ? ' is-selected' : ''),
              'aria-pressed': sel ? 'true' : 'false', 'aria-label': opt.labelJa,
              'data-fkey': key, text: opt.labelJa
            });
            b.addEventListener('click', function () { patch('type', sel ? null : opt.id); draw(); focusBack(panel, key); });
            row.appendChild(b);
          });
          card.appendChild(row);
        });

        // 詳細（程度・範囲・部位）は種類を選んだときだけ展開する
        if (def) {
          [['severity', '程度', D.conditionSeverities], ['extent', '範囲', D.conditionExtents]].forEach(function (tri) {
            card.appendChild(ui.el('p', { class: 'field-sub', text: tri[1] }));
            var row = ui.el('div', { class: 'chips', role: 'group', 'aria-label': tri[1] });
            tri[2].forEach(function (opt) {
              var sel = item[tri[0]] === opt.id;
              var key = 'cond:' + idx + ':' + tri[0] + ':' + opt.id;
              var b = ui.el('button', {
                type: 'button', class: 'chip' + (sel ? ' is-selected' : ''),
                'aria-pressed': sel ? 'true' : 'false', 'aria-label': opt.labelJa,
                'data-fkey': key, text: opt.labelJa
              });
              b.addEventListener('click', function () { patch(tri[0], opt.id); draw(); focusBack(panel, key); });
              row.appendChild(b);
            });
            card.appendChild(row);
          });

          card.appendChild(ui.el('p', { class: 'field-sub', text: '部位（複数可）' }));
          var prow = ui.el('div', { class: 'chips', role: 'group', 'aria-label': '部位' });
          placementPool().forEach(function (pl) {
            var sel = (item.placements || []).indexOf(pl.id) >= 0;
            var key = 'cond:' + idx + ':place:' + pl.id;
            var b = ui.el('button', {
              type: 'button', class: 'chip' + (sel ? ' is-selected' : ''),
              'aria-pressed': sel ? 'true' : 'false', 'aria-label': pl.labelJa,
              'data-fkey': key, text: pl.labelJa
            });
            b.addEventListener('click', function () {
              var ps = (item.placements || []).slice();
              var i = ps.indexOf(pl.id);
              if (i >= 0) ps.splice(i, 1); else ps.push(pl.id);
              patch('placements', ps); draw(); focusBack(panel, key);
            });
            prow.appendChild(b);
          });
          card.appendChild(prow);
          if (item.extent === 'overall' || (item.placements || []).indexOf('overall') >= 0) {
            card.appendChild(ui.el('p', { class: 'field-note', text: '範囲が「全体」のときは、個別の部位指定より全体表現を優先します。' }));
          }
        } else {
          card.appendChild(ui.el('p', { class: 'field-note', text: '種類を選ぶと、程度・範囲・部位を指定できます。' }));
        }
      ...
      panel.appendChild(ui.el('button', {
        type: 'button', class: 'btn btn--sm', text: '＋ 状態・加工を追加',
        'aria-label': '状態・加工を追加',
        'data-fkey': 'cond:add',
        onclick: function () {
          if (items().length >= 2) {
            ui.toast('衣装の状態・加工は2件まで指定できます。');
            return;
          }
          setItems(items().concat([{ type: null, group: 'state', severity: 'moderate', extent: 'localized', placements: [] }]));
          draw();
          focusBack(panel, 'cond:add');
        }
      }));
    }
    draw();
  ...
  routes['/gacha'] = function () {
    ...
    var targets = ui.el('div', { class: 'chips', role: 'group', 'aria-label': '組み替える範囲' });
    ...
    var keeps = ui.el('div', { class: 'chips', role: 'group', 'aria-label': '維持する条件' });
    ...
    if (gachaState.candidates && !gachaState.candidates.length) {
      list.appendChild(ui.el('p', { class: 'p', id: 'gacha-empty', text: '現在の維持条件では、組み替えられる候補がありません。維持する条件を減らすか、別の範囲を選択してください。' }));
    }
    ...
    return screen('', [
      ...
      ui.el('header', { class: 'page-head' }, [
        ...
        ui.el('p', { class: 'p p--note', text: '選択した範囲だけを組み替えます。案を採用するまで、現在の設計には反映されません。' })
      ]),
      ui.el('h3', { class: 'sub-head', text: '組み替える範囲' }),
      targets,
      ui.el('h3', { class: 'sub-head', text: '維持する条件' }),
      keeps,
      actions,
      list
    ]);
  ...
  function structureLines(o) {
    ...
    o.condition.items.forEach(function (it, i) {
      if (!it.type) return;
      var def = util.byId(D.conditions, it.type);
      push('状態・加工 ' + (i + 1), [
        util.labelOf(D.conditions, it.type) + (def && def.group === 'treatment' ? '（加工）' : ''),
        '程度：' + util.labelOf(D.conditionSeverities, it.severity),
        '範囲：' + util.labelOf(D.conditionExtents, it.extent),
        (it.placements || []).length
          ? '部位：' + it.placements.map(function (pid) { return util.labelOf(D.conditionPlacements, pid); }).join('・')
          : '部位：未指定'
      ].join(' / '));
    });
    ...
    lib.forEach(function (o) {
      ...
      var card = ui.el('div', { class: 'card' }, [
        ...
        ui.el('div', { class: 'row' }, [
          ...
          ui.el('button', {
            class: 'btn btn--sm', type: 'button', text: 'データ出力',
            'aria-label': (o.name || '無題') + 'の設計データを書き出す',
            onclick: function () { downloadJSON(store.exportOutfit(o), (o.name || 'outfit') + '.json'); }
          }),
          ui.el('button', {
  ...
  function downloadJSON(text, filename) {
    try {
      ...
      ui.toast('設計データを書き出しました。');
    } catch (e) { ui.toast('設計データの書き出しに失敗しました。'); }
  }
