...
(function (global) {
  ...
  var schema = (CPW.schema = {
    ...
    createOutfit: function () {
      return {
        ...
        output: { includeNarrative: false, includeEffects: false, includePresentation: false, includeQualityTags: false }
      };
    },
  ...
  var CONCEPT_FIELDS = [
    ...
    { key: 'concept.secondaryStyles', labelJa: '副様式', type: 'multi', optional: true, options: opts(D.styles), noteJa: '複数選べるけれど、主様式を1つ決めると輪郭が安定するよ。' },
    {
      key: 'concept.primaryThemeMotif', labelJa: '主テーマ', type: 'single', optional: true,
      options: opts(D.motifs),
      noteJa: '衣装全体の題材。世界観・配色・素材・候補提案へ広く効く。装飾の形そのものは「主役装飾モチーフ」で指定するよ。',
      grouped: true
    },
  ...
  function renderAttributeField() {
    ...
    var draw = function () {
      ...
      body.appendChild(ui.el('p', { class: 'field-note', text: '「魔法的演出」を切っている間は、炎や氷のエフェクトを出力に足さないよ。' }));
  ...
  function renderCompositeSlot(slot) {
    ...
    function draw() {
      ...
      if (val[reqKey]) {
        ...
        if (held) body.appendChild(ui.el('p', { class: 'field-note field-note--data', text: '素材・長さ・指先は、出力ではひとつの句にまとめるよ。' }));
      } else {
        ...
        if (dormant.length) {
          body.appendChild(ui.el('p', { class: 'field-note field-note--data', text: '休止中：' + dormant.join('・') + '。種類を選び直すと、この指定がそのまま戻るよ。出力には混ざらない。' }));
        }
      }
    }
    draw();
  ...
  function buildSpecialSection(panel) {
    function draw() {
      ...
      panel.appendChild(ui.el('p', {
        class: 'p p--note',
        text: '翼・角・光輪・尾は「種類」が要。種類を選ぶと、その下の項目が編集できるようになるよ。種類を外すと、指定は残したまま休止して、出力・完成度・警告には混ざらない。'
      }));
      ...
      panel.appendChild(renderSpecialList({
        ...
        noteJa: '衣装の飾りとしての鎖。「衣装のみ」の出力でも出るよ。',
      ...
      panel.appendChild(renderSpecialList({
        ...
        noteJa: '身の回りに浮いている物。光そのものではないので、エフェクトOFFでも出るよ。',
  ...
  function renderSpecialSlot(slot, redraw) {
    ...
    slot.axes.forEach(function (axis) {
      ...
      if (axis.key === 'color' && val.color) {
        ...
        if (opt && opt.individual) {
          body.appendChild(ui.el('p', { class: 'field-note', text: 'この部位だけの色を選んでね。' }));
        ...
        if (opt && opt.linkTo) {
          body.appendChild(ui.el('p', {
            ...
            text: resolved
              ? '連動中：いまは「' + resolved.labelJa + '」。衣装側の色を変えると、ここも一緒に動くよ。'
              : '連動先の色がまだ決まっていないよ。配色を決めると色が入る。'
          }));
        }
      }
    });
    ...
    if (val[reqKey]) {
      body.appendChild(ui.el('p', { class: 'field-note field-note--data', text: '出力では、ひとつの句にまとめるよ。' }));
      var del = ui.el('button', { type: 'button', class: 'btn btn--small btn--ghost', 'data-fkey': 'sp:clear:' + slot.id, text: slot.labelJa + 'を外す' });
      del.addEventListener('click', function () {
        if (!ui.confirm(slot.labelJa + 'の指定をすべて消すよ。いい？ この操作は元に戻せない。')) return;
      ...
      body.appendChild(ui.el('p', {
        class: 'field-note field-note--data',
        text: '休止中：' + held.join('・') + '。種類を選び直すと、この指定がそのまま戻るよ。いまは出力に混ざらない。'
      }));
    }
  ...
  function renderSpecialList(def, redraw) {
    ...
    if (def.gatedBy && !state.outfit.output[def.gatedBy] && list.length) {
      wrap.appendChild(ui.el('p', {
        ...
        text: 'いまの出力設定では、ここは英語に出ないよ。出したいときは出力の「物語」をONにしてね。'
      }));
    }
  ...
  function buildStructureSection(panel) {
    function draw() {
      ...
      panel.appendChild(selectField({
        ...
        noteJa: '選んだ大分類によって、下に出る部位が切り替わるよ。',
      ...
      if (!cat) {
        panel.appendChild(ui.el('p', { class: 'p p--note', text: '大分類を選ぶと、その服に要る部位だけがここに出るよ。' }));
        return;
      }
  ...
  function buildDecorations(host) {
    ...
    function draw() {
      ...
      dens.appendChild(ui.el('p', {
        ...
        text: '選んだ装飾の量：' + used + ' / この密度の目安：' + budget + (used > budget ? '（目安を超えているよ。Phase 4で飽和警告を出す）' : '')
      }));
      ...
      list.appendChild(ui.el('p', { class: 'field-note', text: '装飾は「種類」と「位置」をまとめて選ぶのではなく、一件ごとに紐付けるよ。' }));
      ...
      items().forEach(function (item, idx) {
        ...
        if (d && (item.placements || []).length === 0) {
          card.appendChild(ui.el('p', { class: 'field-note', text: '位置が空のままだと、どこに付くか伝わらないよ。' }));
        }
  ...
  function buildPaletteSection(panel) {
    ...
    function draw() {
      ...
      prev.appendChild(any ? bar : ui.el('p', { class: 'p p--note', text: 'まだ色が決まっていないよ。主色だけでも出力はできる。' }));
      if (any) prev.appendChild(ui.el('p', { class: 'field-note', text: '面積の目安は 主色70・副色20・差し色10。数値の入力は要らないよ。' }));
      ...
      D.colorRoles.forEach(function (role) {
        ...
        if (openRole === role.id) {
          ...
          if (!list.length) picker.appendChild(ui.el('p', { class: 'p p--note', text: 'この絞り込みに合う色がないよ。' }));
  ...
  function backBar(label, href) {
    return ui.el('div', { class: 'backbar' }, [
      ui.el('a', { class: 'back-link', href: href, text: '← ' + label })
    ]);
  }
  ...
  routes[''] = function () {
    ...
    return screen('', [
      ...
      ui.el('div', { class: 'card card--quiet' }, [
        ...
        ui.el('p', { class: 'p', text: '設計はこの端末のブラウザ内だけに保存されるよ。閲覧履歴やサイトデータを消すと一緒に消えるから、大事な一着はJSONで書き出して手元に置いておくのが安全。' }),
        store.available() ? null : ui.el('p', { class: 'p p--warn', text: 'このブラウザでは端末内保存が使えない設定になっているみたい。設計は保存されないよ。' })
      ])
    ]);
  };
  ...
  /* ---------- 開始方法 ---------- */
  var ENTRY_MODES = [
    { id: 'zero', labelJa: 'ゼロから設計', descJa: '空の状態から、全部を自分で選ぶ。' },
    { id: 'worldview', labelJa: '世界観から設計', descJa: '現代・ファンタジー・和風などを決めて、合う候補を優先表示する。' },
    { id: 'garment', labelJa: '基本衣装から設計', descJa: 'ドレス、制服、水着など、服の形を起点にする。' },
    { id: 'preset', labelJa: 'プリセットから設計', descJa: '整合した初期値を読み込む。読み込んだ後は全部変えられる。' },
    { id: 'motif', labelJa: 'モチーフから設計', descJa: '童話・星座・神話・属性など、題材を起点にする。' }
  ];
  ...
  routes['/entry'] = function () {
    ...
    return screen('', [
      ...
      ui.el('p', { class: 'p p--lead', text: 'ここで選んだ内容は初期値として設計台へ入るだけ。後からどれでも変えられるよ。' }),
  ...
  routes['/setup'] = function (params) {
    ...
    if (mode === 'zero') {
      body.appendChild(ui.el('p', { class: 'p p--lead', text: '何も置かない状態から始めるよ。設計台のどの項目からでも触れる。' }));
      body.appendChild(ui.el('button', { class: 'btn btn--primary', type: 'button', onclick: function () { start(); }, text: '設計台をひらく' }));
    }

    if (mode === 'worldview') {
      body.appendChild(ui.el('p', { class: 'p p--lead', text: '世界観を1つ選ぶと、それに合う候補が優先して並ぶよ。' }));
    ...
    if (mode === 'garment') {
      body.appendChild(ui.el('p', { class: 'p p--lead', text: '服の形から始めるよ。選んだ大分類に応じて、設計台に出る部位が切り替わる。' }));
    ...
    if (mode === 'preset') {
      body.appendChild(ui.el('p', { class: 'p p--lead', text: '整合の取れた初期値をまとめて読み込むよ。読み込んだ後は全部変えられる。' }));
    ...
    if (mode === 'motif') {
      body.appendChild(ui.el('p', { class: 'p p--lead', text: '題材を1つ選んで主テーマにするよ。副テーマは設計台で足せる。' }));
      body.appendChild(ui.el('p', { class: 'p p--note', text: '神話や文化の意匠は、断定的な再現ではなく創作衣装のための着想として扱っているよ。' }));
  ...
  function buildHeaderInner() {
    ...
    var wrap = ui.el('div', {}, [
      ui.el('div', { class: 'wh-top' }, [
        ui.el('a', { class: 'back-link', href: '#/', text: '← トップ' }),
      ...
      (function () {
        var input = ui.el('input', {
          ...
          placeholder: '衣装名（例：幽閉された王子）', 'aria-label': '衣装名',
          maxlength: '60'
        });
        input.value = o.name || '';
        input.addEventListener('input', function () {
          ...
          state.dirty = true;
          autosave();
          var b = document.querySelector('#workshop-header .badge');
      ...
      ui.el('button', {
        ...
        onclick: function () {
          if (!state.outfit.name) { ui.toast('衣装名を入れてから保存してね'); var n = document.getElementById('outfit-name'); if (n) n.focus(); return; }
          state.saveToLibrary();
          ui.toast('「' + state.outfit.name + '」を保存したよ');
        }
      })
    ]);
    return wrap;
  ...
  routes['/workshop'] = function () {
    ...
    return screen('', [header, suggestLink, body, bar]);
  };
    ...
    if (issue.ignored) {
      ...
      actions.appendChild(ui.el('button', {
        ...
        onclick: function () {
          ...
          ui.toast(issue.severity === 'hard' ? '維持したよ。ただし同じ層での競合は生成が崩れやすい。' : '維持したよ');
          render();
        }
      }));
    }
  ...
  routes['/checks'] = function () {
    ...
    if (!live.length && !muted.length) {
      body.appendChild(ui.el('p', { class: 'p', id: 'no-issue', text: '気になる組み合わせは見つからなかったよ。' }));
    }
  ...
  routes['/suggest'] = function () {
    ...
    if (!cands.length) {
      list.appendChild(ui.el('p', { class: 'p', id: 'no-cand', text: 'いまの設計に足せる候補は見つからなかったよ。先に世界観や基本衣装を決めると出やすい。' }));
    }
  ...
  function copyText(text, whatJa) {
    if (!text) { ui.toast('コピーする中身がまだ無いよ'); return; }
    var fallback = function () {
      try {
        ...
        if (ok) ui.toast(whatJa + 'をコピーしたよ');
        else ui.toast('コピーできなかったよ。長押しして選んでね');
      } catch (e) {
        ui.toast('コピーできなかったよ（' + (e && e.message ? e.message : '理由不明') + '）。長押しして選んでね');
      }
    };
    try {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(function () { ui.toast(whatJa + 'をコピーしたよ'); }, fallback);
      } else fallback();
    } catch (e) { fallback(); }
  }
  ...
  routes['/output'] = function () {
    ...
    function draw() {
      ...
      host.appendChild(ui.el('p', { class: 'p p--note', text: onlyCostume ? 'いまは「衣装のみ」。背景・ポーズ・感情・物語・魔法演出・品質タグは入っていないよ。' : '任意オプションが入った状態の出力だよ。' }));
      ...
      if (outputTab === 'structure') {
        ...
        if (!lines.length) table.appendChild(ui.el('dd', { class: 'p p--note', text: 'まだ何も選ばれていないよ。設計台で項目を選ぶと、ここに一覧が出る。' }));
      ...
      var opts_ = ui.el('div', { class: 'card card--quiet' }, [
        ...
        ui.el('p', { class: 'p p--sub', text: '既定は衣装だけ。要るものだけを足してね。' })
      ]);
  ...
  routes['/library'] = function () {
    ...
    if (!lib.length) {
      body.appendChild(ui.el('div', { class: 'card card--quiet' }, [
        ui.el('p', { class: 'p', text: 'まだ保存した衣装がないよ。設計台で名前を付けて保存すると、ここに並ぶ。' }),
        ui.el('a', { class: 'btn btn--primary', href: '#/entry', text: '新しい衣装を設計する' })
      ]));
    }
    ...
    lib.forEach(function (o) {
      ...
      var card = ui.el('div', { class: 'card' }, [
        ...
        ui.el('div', { class: 'row' }, [
          ...
          ui.el('button', { class: 'btn btn--sm', type: 'button', text: '複製', onclick: function () { store.duplicateOutfit(o.id); ui.toast('複製したよ'); render(); } }),
          ...
          ui.el('button', {
            ...
            onclick: function () {
              if (!ui.confirm('「' + (o.name || '名称未設定の衣装') + '」を削除する？ この操作は元に戻せないよ。')) return;
    ...
    importInput.addEventListener('change', function () {
      ...
      reader.onload = function () {
        ...
        else { ui.toast(r.imported + '件を取り込んだよ' + (r.skipped ? '（' + r.skipped + '件は読めず飛ばした）' : '')); render(); }
        importInput.value = '';
      };
      reader.onerror = function () { ui.toast('ファイルを読めなかったよ'); importInput.value = ''; };
    ...
    return screen('', [
      ...
      ui.el('div', { class: 'card card--quiet' }, [
        ...
        ui.el('p', { class: 'p', text: '端末内保存は、ブラウザのデータを消すと一緒に消えるよ。JSONで書き出しておけば、別の端末でも読み込める。' }),
        ui.el('div', { class: 'row' }, [
