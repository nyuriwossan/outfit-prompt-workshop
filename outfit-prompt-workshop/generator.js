...
(function (global) {
  ...
  function themeAdj(o) {
    ...
    if (!m) return null;
    if (m.themeAdjEn) return m.themeAdjEn;
    return (m.shortPrompt || '').replace(/\s*motifs?$/, '') + '-inspired';
  }
  ...
  function blockIdentity(o, mode) {
    ...
    return {
      ...
      detailed: [withArticle(dropRedundant(common, headDetail).concat([headDetail]).join(' '))]
    };
  }
  ...
  function blockMaterials(o) {
    var m = o.materials;
    var primary = en(D.materials, m.primary);
    var secondary = en(D.materials, m.secondary);
    var trim = en(D.materials, m.trim);
    ...
    if (secondary) sh.push(secondary + ' details');
    ...
    if (trim) extras.push(trim);
    if (extras.length) de.push('combined with ' + joinAnd(extras));
  ...
  function blockTheme(o) {
    ...
    if (moodList.length) de.push(joinAnd(moodList) + ' in mood');
    return { short: sh, detailed: [de.join(', ')] };
  }
  ...
  function blockOptionalNarrative(o) {
    ...
    if (themeWords.length) {
      ...
      out.push(article(t) + ' ' + t + ' theme');
    } else if (uniqMoods.length) {
      out.push(article(uniqMoods[0]) + ' ' + uniqMoods[0] + ' mood');
    }
    if (!out.length) return { short: [], detailed: [] };
   ...
   * ========================================================== */
  var BLOCK_ORDER = [
    'identity', 'palette', 'silhouette', 'garments', 'parts', 'materials',
  ...
  var EMPTY_JA = '（まだ出力できる項目がないよ。設計台で基本衣装か主色を選ぶと、ここに英語が出る。）';
  ...
  var CONNECTORS = {
    ...
    optionalNarrative: ['carrying', 'set in'],
    quality: ['', '']
  };
