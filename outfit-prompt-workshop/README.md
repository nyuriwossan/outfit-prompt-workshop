...
`tests.html` をブラウザで開くと全件走る。**Phase 5A 時点で 390 件、全通過。**

内訳：Phase 1〜3 が 172 件（util・スキーマ・検証・移行・パッチ・完成度・保存・書き出し・ルーター・データ整合・手袋の4軸・レッグウェアの層・生成エンジン・ケースA〜D）。Phase 4 で 102 件（競合判定・配色・装飾密度・補助候補・部分ガチャ）。Phase 5A で 116 件を追加。
