...
(function (global) {
  ...
  /* ============================================================
   * Phase 5B 後半：衣装の状態・加工
   * ========================================================== */
  function condOutfit(patch) {
    return SC.applyPatch(SC.createOutfit(), patch || {});
  }
  function condItem(type, over) {
    var base = { type: type, group: 'state', severity: 'moderate', extent: 'localized', placements: [] };
    Object.keys(over || {}).forEach(function (k) { base[k] = over[k]; });
    return base;
  }

  /* ---- データ健全性 ---- */
  test('5B後半：condition IDが重複していない', function () {
    var seen = {};
    D.conditions.forEach(function (c) {
      assert(!seen[c.id], '重複ID：' + c.id);
      seen[c.id] = true;
    });
  });
  test('5B後半：condition IDはsnake_case', function () {
    D.conditions.concat(D.conditionPlacements, D.conditionSeverities, D.conditionExtents).forEach(function (c) {
      assert(/^[a-z][a-z0-9_]*$/.test(c.id), 'snake_caseでない：' + c.id);
    });
  });
  test('5B後半：labelJa・base・groupが空でない', function () {
    D.conditions.forEach(function (c) {
      assert(c.labelJa && c.base, '空のlabel/base：' + c.id);
      assert(c.group === 'state' || c.group === 'treatment', 'groupが不正：' + c.id + '=' + c.group);
      assert(U.byId(D.conditionFamilies, c.family), 'familyが不正：' + c.id);
    });
  });
  test('5B後半：byClass・avoidClassesのキーが既知の素材分類', function () {
    var known = ['textile', 'knit', 'leather', 'fur', 'plush', 'rigid', 'metallic', 'waterproof', 'scales'];
    D.conditions.forEach(function (c) {
      Object.keys(c.byClass || {}).forEach(function (k) { assert(known.indexOf(k) >= 0, c.id + ' byClass不明：' + k); });
      (c.avoidClasses || []).forEach(function (k) { assert(known.indexOf(k) >= 0, c.id + ' avoidClasses不明：' + k); });
    });
  });
  test('5B後半：仕様書の必須状態が最低限そろっている', function () {
    ['dust_stained', 'mud_splattered', 'soiled', 'soot_stained', 'oil_stained', 'sand_covered', 'salt_stained', 'paint_splattered',
     'frayed_edges', 'torn_light', 'torn_places', 'tattered', 'scuffed', 'scratched', 'dented', 'chipped', 'singed', 'scorched', 'patched', 'battle_worn',
     'slightly_damp', 'rain_soaked', 'water_soaked', 'drenched', 'water_droplets', 'sea_spray', 'snow_damp',
     'blood_stain_small', 'blood_spattered', 'blood_soaked', 'blood_drenched', 'blood_smeared',
     'worn', 'faded', 'sun_faded', 'wrinkled', 'weathered', 'aged',
     'distressed', 'stone_washed', 'faded_wash', 'bleach_washed', 'patchwork', 'repair_stitching', 'raw_edged', 'crackle_finished'
    ].forEach(function (id) { assert(U.byId(D.conditions, id), '必須の状態が無い：' + id); });
    assert(D.conditions.length >= 45, '状態・加工が少なすぎる：' + D.conditions.length);
  });
  test('5B後半：treatment群はすべてgroup=treatment', function () {
    D.conditions.filter(function (c) { return c.family === 'treatment'; }).forEach(function (c) {
      eq(c.group, 'treatment', c.id);
    });
  });
  test('5B後半：部位に仕様書の最低限（裾〜衣装全体）がある', function () {
    ['hem', 'sleeves', 'cuffs', 'shoulders', 'chest', 'back', 'knees', 'overall'].forEach(function (id) {
      assert(U.byId(D.conditionPlacements, id), '部位が無い：' + id);
    });
  });
  test('5B後半：状態付きプリセットの参照先がすべて実在する', function () {
    D.presets.forEach(function (pr) {
      var items = (pr.patch.condition && pr.patch.condition.items) || [];
      items.forEach(function (it) {
        assert(U.byId(D.conditions, it.type), pr.id + ' の状態が不明：' + it.type);
        assert(U.byId(D.conditionSeverities, it.severity), pr.id + ' の程度が不明');
        assert(U.byId(D.conditionExtents, it.extent), pr.id + ' の範囲が不明');
        (it.placements || []).forEach(function (pl) {
          assert(U.byId(D.conditionPlacements, pl), pr.id + ' の部位が不明：' + pl);
        });
      });
    });
    assert(D.presets.filter(function (pr) { return pr.group === 'scene'; }).length >= 8, '状態付きプリセットが8件未満');
    assert(D.presets.length >= 32, 'プリセット総数が減っている');
  });
  ...
  test('5B後半：0.2データ（人魚）を読み込める', function () {
    var m = SC.migrate({ version: '0.2', concept: {}, garment: { category: 'merfolk', subtype: 'mermaid_tail' },
      parts: { mermaid_tail_form: 'classic_scaled_tail' } });
    assert(m.ok, m.reasonJa);
    eq(m.outfit.parts.mermaid_tail_form, 'classic_scaled_tail');
    eq(m.outfit.condition.items.length, 0);
  });
  test('5B後半：condition無し・customTags無しの旧データでも壊れない', function () {
    var m = SC.migrate({ concept: {}, garment: { category: 'dress', subtype: 'ball_gown' } });
    assert(m.ok);
    eq(m.outfit.output.customTags, '');
    eq(m.outfit.condition.items.length, 0);
  });
  test('5B後半：未知のcondition IDは捨てて停止しない', function () {
    var o = SC.normalize({ concept: {}, garment: { category: 'dress', subtype: 'ball_gown' },
      condition: { items: [condItem('no_such_condition'), condItem('mud_splattered')] } });
    eq(o.condition.items.length, 1);
    eq(o.condition.items[0].type, 'mud_splattered');
    var s = G.short(o);
    assert(s.indexOf('undefined') < 0, s);
  });
  test('5B後半：no_conditionのような疑似IDは保存されない', function () {
    var o = SC.normalize({ concept: {}, garment: {}, condition: { items: [condItem('no_condition')] } });
    eq(o.condition.items.length, 0);
  });
  test('5B後半：severity・extent・placementsは既知値へ丸める', function () {
    var o = SC.normalize({ concept: {}, garment: {},
      condition: { items: [{ type: 'soiled', group: 'state', severity: 'ultra', extent: 'everywhere', placements: ['hem', 'hem', 'nowhere'] }] } });
    eq(o.condition.items[0].severity, 'moderate');
    eq(o.condition.items[0].extent, 'localized');
    eq(o.condition.items[0].placements.join(','), 'hem');
  });
  test('5B後半：3件以上は先頭2件に丸める（ケースN相当の保存側保証）', function () {
    var o = SC.normalize({ concept: {}, garment: {},
      condition: { items: [condItem('soiled'), condItem('worn'), condItem('faded')] } });
    eq(o.condition.items.length, 2);
    eq(o.condition.items[0].type, 'soiled');
    eq(o.condition.items[1].type, 'worn');
  });
  test('5B後半：状態付き衣装のJSON往復', function () {
    var o = condOutfit({ garment: { category: 'dress', subtype: 'shirt_dress' }, materials: { primary: 'cotton' },
      condition: { items: [condItem('mud_splattered', { severity: 'light', placements: ['hem'] })] },
      output: { customTags: 'masterpiece, BestQuality' } });
    var back = SC.migrate(JSON.parse(JSON.stringify(o)));
    assert(back.ok);
    eq(back.outfit.condition.items[0].type, 'mud_splattered');
    eq(back.outfit.condition.items[0].placements[0], 'hem');
    eq(back.outfit.output.customTags, 'masterpiece, BestQuality');
  });
  test('5B後半：merfolk＋condition のJSON往復', function () {
    var o = condOutfit({ garment: { category: 'merfolk', subtype: 'mermaid_tail' },
      parts: { mermaid_tail_form: 'classic_scaled_tail' },
      condition: { items: [condItem('water_droplets', { placements: ['mermaid_tail'] })] } });
    var back = SC.migrate(JSON.parse(JSON.stringify(o)));
    assert(back.ok);
    eq(back.outfit.garment.category, 'merfolk');
    eq(back.outfit.condition.items[0].type, 'water_droplets');
    eq(back.outfit.condition.items[0].placements[0], 'mermaid_tail');
  });
  test('5B後半：旧スロットID＋conditionの同時移行', function () {
    var m = SC.migrate({ version: '0.1', concept: {}, garment: { category: 'dress', subtype: 'ball_gown' },
      parts: { skirtShape: 'a_line_skirt' },
      condition: { items: [condItem('worn')] } });
    assert(m.ok);
    eq(m.outfit.parts.skirt_shape, 'a_line_skirt');
    eq(m.outfit.condition.items[0].type, 'worn');
  });

  /* ---- 英文生成（受入ケースA〜P） ---- */
  test('5BケースA：裾の泥はね', function () {
    var o = condOutfit({ garment: { category: 'dress', subtype: 'ball_gown' }, materials: { primary: 'silk' },
      condition: { items: [condItem('mud_splattered', { severity: 'light', placements: ['hem'] })] } });
    var d = G.detailed(o);
    assert(d.indexOf('lightly mud-splattered along the hem') >= 0, d);
    assert(d.indexOf('with dirt') < 0, d);
    assert((d.match(/hem/g) || []).length === 1, '部位が重複：' + d);
  });
  test('5BケースB：片袖の軽い破れ', function () {
    var o = condOutfit({ garment: { category: 'top_bottom', subtype: 'simple_tshirt_and_jeans' }, materials: { primary: 'cotton' },
      condition: { items: [condItem('torn_light', { severity: 'light', placements: ['one_sleeve'] })] } });
    var d = G.detailed(o);
    assert(d.indexOf('slightly torn at one sleeve') >= 0, d);
    assert(d.indexOf('torn open') < 0, d);
    assert(!/nude|naked|bare (chest|skin)|underwear/i.test(d), '身体・下着の自動追加：' + d);
  });
  test('5BケースC：金属鎧はtorn/frayed/wrinkledにならない', function () {
    var o = condOutfit({ garment: { category: 'uniform', subtype: 'royal_uniform' }, materials: { primary: 'plate_armor' },
      condition: { items: [condItem('torn_places', { severity: 'heavy', placements: ['armor_surface'] }), condItem('wrinkled')] } });
    var d = G.detailed(o) + ' ' + G.short(o);
    assert(/scratched|dented|chipped|battle-worn/.test(d), d);
    assert(!/torn plate armor|frayed (metal|armor)|wrinkled armor/.test(d), d);
    assert(d.indexOf('torn') < 0, '金属にtornが残る：' + d);
    assert(d.indexOf('wrinkled') < 0, '金属にwrinkledが残る：' + d);
  });
  test('5BケースD：雨濡れから透け・密着・露出を自動追加しない', function () {
    var o = condOutfit({ garment: { category: 'dress', subtype: 'shirt_dress' },
      materials: { primary: 'cotton', transparency: 'opaque' },
      condition: { items: [condItem('rain_soaked', { severity: 'heavy', extent: 'widespread' })] } });
    var d = G.detailed(o) + ' ' + G.short(o);
    assert(/rain-soaked|drenched with water/.test(d), d);
    assert(!/see-through|translucent|clinging|skin-tight from|transparent when wet|exposed/i.test(d), '透け・密着の自動追加：' + d);
    assert(d.indexOf('with wet') < 0, d);
  });
  test('5BケースE：白衣の血痕は衣装への付着として出る', function () {
    var o = condOutfit({ garment: { category: 'robe', subtype: 'laboratory_coat_outfit' }, materials: { primary: 'cotton' },
      condition: { items: [condItem('blood_stain_small', { severity: 'light', placements: ['one_sleeve'] })] } });
    var d = G.detailed(o);
    assert(/blood-stained.*one sleeve/.test(d), d);
    assert(!/wound|injur|bleeding|gore|entrails|viscera/i.test(d), '負傷・goreの自動追加：' + d);
  });
  test('5BケースF：大量の血濡れも衣装の状態として出る', function () {
    var o = condOutfit({ garment: { category: 'dress', subtype: 'gothic_dress' }, materials: { primary: 'velvet' },
      condition: { items: [condItem('blood_drenched', { severity: 'heavy', extent: 'overall' })] } });
    var d = G.detailed(o) + ' ' + G.short(o);
    assert(/drenched in blood|blood-soaked/.test(d), d);
    assert(!/wound|injur|bleeding from|gore/i.test(d), d);
  });
  test('5BケースG：使用感は戦闘描写へ寄らない', function () {
    var o = condOutfit({ garment: { category: 'top_bottom', subtype: 'simple_tshirt_and_jeans' }, materials: { primary: 'cotton' },
      condition: { items: [condItem('worn', { severity: 'light' })] } });
    var d = G.detailed(o) + ' ' + G.short(o);
    assert(/well-worn|visibly worn/.test(d), d);
    assert(!/battle|combat|torn/.test(d), d);
  });
  test('5BケースH：デニムのダメージ加工は加工のまま出る', function () {
    var o = condOutfit({ garment: { category: 'top_bottom', subtype: 'simple_tshirt_and_jeans' }, materials: { primary: 'denim' },
      condition: { items: [{ type: 'distressed', group: 'treatment', severity: 'moderate', extent: 'localized', placements: ['knees'] }] } });
    var d = G.detailed(o) + ' ' + G.short(o);
    assert(d.indexOf('distressed') >= 0, d);
    assert(!/battle|torn in several/.test(d), '戦闘由来へ変換された：' + d);
  });
  test('5BケースI：防水素材は水を吸わない', function () {
    var o = condOutfit({ garment: { category: 'top_bottom', subtype: 'hoodie_and_cargo' }, materials: { primary: 'nylon' },
      condition: { items: [condItem('water_soaked')] } });
    var d = G.detailed(o) + ' ' + G.short(o);
    assert(/water droplets|rain-beaded|water running across the surface/.test(d), d);
    assert(!/soaked through|water-absorbed|water-soaked/.test(d), d);
  });
  test('5BケースJ：毛皮は matted / wet / mud-stained 系で出る', function () {
    var o = condOutfit({ garment: { category: 'robe', subtype: 'fur_coat' }, materials: { primary: 'faux_fur' },
      condition: { items: [condItem('rain_soaked'), condItem('mud_splattered', { severity: 'light', placements: ['hem'] })] } });
    var d = G.detailed(o);
    assert(/matted fur/.test(d), d);
    assert(/mud-stained/.test(d), d);
    assert(d.indexOf('torn') < 0, '布と同じtornへ固定：' + d);
  });
  test('5BケースK：人魚の水滴は鱗の表現になり脚・靴が出ない', function () {
    var o = condOutfit({ garment: { category: 'merfolk', subtype: 'mermaid_tail' },
      parts: { mermaid_tail_form: 'classic_scaled_tail' }, materials: { primary: 'iridescent_fabric' },
      condition: { items: [condItem('water_droplets', { severity: 'light', placements: ['mermaid_tail'] })] } });
    var d = G.detailed(o);
    assert(/water-slick scales|glistening with water/.test(d), d);
    var cleaned = d.replace(/no feet/g, '').replace(/no human legs/g, '').replace(/replacing separate human legs and feet/g, '');
    assert(!/\b(feet|foot|legs?|boots?|shoes)\b/i.test(cleaned), '脚・靴の混入：' + d);
  });
  test('5BケースK2：人魚詳細版でmermaid tailが一度しか出ない（重複修正）', function () {
    var o = condOutfit({ garment: { category: 'merfolk', subtype: 'mermaid_tail' },
      parts: { mermaid_tail_form: 'classic_scaled_tail' }, materials: { primary: 'iridescent_fabric' } });
    var d = G.detailed(o);
    // 衣装の核（mermaid tail outfit）は数えず、「尾を説明する句」が一つに統合されていることを見る。
    eq((d.match(/scaled mermaid tail/g) || []).length, 1, '尾の説明句が重複：' + d);
    assert(d.indexOf('with the lower body ending in a single classic scaled mermaid tail') >= 0, '尾の形が統合されていない：' + d);
    assert(d.indexOf('with classic scaled mermaid tail,') < 0, '旧形式のwith句が残る：' + d);
  });
  test('5BケースL：人魚の傷は鱗の傷になり獣の尾と混同しない', function () {
    var o = condOutfit({ garment: { category: 'merfolk', subtype: 'mermaid_tail' },
      parts: { mermaid_tail_form: 'classic_scaled_tail' }, materials: { primary: 'iridescent_fabric' },
      condition: { items: [condItem('scratched', { placements: ['mermaid_tail'] }), condItem('chipped', { severity: 'light', placements: ['tail_fin'] })] } });
    var d = G.detailed(o) + ' ' + G.short(o);
    assert(/scratched scales/.test(d), d);
    assert(/chipped scales/.test(d), d);
    assert(!/fox tail|wolf tail|cat tail/.test(d), d);
  });
  test('5BケースM：状態2件が自然な順序で並び句読点が壊れない', function () {
    var o = condOutfit({ garment: { category: 'dress', subtype: 'shirt_dress' }, materials: { primary: 'cotton' },
      condition: { items: [condItem('mud_splattered', { severity: 'light', placements: ['hem'] }), condItem('torn_light', { severity: 'light', placements: ['one_sleeve'] })] } });
    var d = G.detailed(o);
    assert(d.indexOf('lightly mud-splattered along the hem') >= 0, d);
    assert(d.indexOf('slightly torn at one sleeve') >= 0, d);
    assert(!/,\s*,|\.\s*,|\.\./.test(d), '句読点の破損：' + d);
  });
  test('5BケースO：treatmentは状態より先に出る（並びの安定）', function () {
    var o = condOutfit({ garment: { category: 'top_bottom', subtype: 'simple_tshirt_and_jeans' }, materials: { primary: 'denim' },
      condition: { items: [condItem('mud_splattered', { severity: 'light', placements: ['hem'] }),
        { type: 'distressed', group: 'treatment', severity: 'moderate', extent: 'localized', placements: [] }] } });
    var d = G.detailed(o);
    assert(d.indexOf('distressed') < d.indexOf('mud-splattered'), '加工が状態の後にある：' + d);
  });
  test('5BケースP：状態文の後に追加タグを無改変で付与', function () {
    var o = condOutfit({ garment: { category: 'dress', subtype: 'shirt_dress' }, materials: { primary: 'cotton' },
      condition: { items: [condItem('mud_splattered', { severity: 'light', placements: ['hem'] })] },
      output: { customTags: 'masterpiece, BestQuality' } });
    var d = G.detailed(o);
    assert(/mud-splattered along the hem\./.test(d), d);
    assert(d.indexOf('masterpiece, BestQuality') === d.length - 'masterpiece, BestQuality'.length, 'タグが末尾で無改変になっていない：' + d);
    assert(d.indexOf('. ,') < 0 && d.indexOf(',,') < 0, d);
  });
  test('5B後半：状態が空なら状態ブロックも接続語も出ない', function () {
    var o = condOutfit({ garment: { category: 'dress', subtype: 'shirt_dress' }, materials: { primary: 'cotton' } });
    var b = G.blocks(o);
    eq(b.condition.short.length, 0);
    eq(b.condition.detailed.length, 0);
  });
  test('5B後半：範囲「全体」で torn at overall のような連結を作らない', function () {
    var o = condOutfit({ garment: { category: 'dress', subtype: 'shirt_dress' }, materials: { primary: 'cotton' },
      condition: { items: [condItem('soiled', { severity: 'heavy', extent: 'overall', placements: ['hem'] })] } });
    var d = G.detailed(o);
    assert(d.indexOf('heavily soiled throughout') >= 0, d);
    assert(d.indexOf('at overall') < 0 && d.indexOf('hem') < 0, '全体指定なのに部位が残る：' + d);
  });
  test('5B後半：広範囲＋背中は across much of the back になる', function () {
    var o = condOutfit({ garment: { category: 'dress', subtype: 'shirt_dress' }, materials: { primary: 'cotton' },
      condition: { items: [condItem('blood_stains_marked', { extent: 'widespread', placements: ['back'] })] } });
    var d = G.detailed(o);
    assert(d.indexOf('across much of the back') >= 0, d);
  });
  test('5B後半：程度副詞はwith句の内側に入る', function () {
    var o = condOutfit({ garment: { category: 'merfolk', subtype: 'mermaid_tail' },
      parts: { mermaid_tail_form: 'classic_scaled_tail' }, materials: { primary: 'iridescent_fabric' },
      condition: { items: [condItem('chipped', { severity: 'light', placements: ['tail_fin'] })] } });
    var d = G.detailed(o) + ' ' + G.short(o);
    assert(d.indexOf('lightly with') < 0, '副詞がwithの外にある：' + d);
    assert(/with lightly chipped scales|lightly chipped scales/.test(d), d);
  });

  test('5B監査固定：人魚に脚系部位のconditionが入っても出力しない', function () {
    var o = condOutfit({ garment: { category: 'merfolk', subtype: 'mermaid_tail' },
      parts: { mermaid_tail_form: 'classic_scaled_tail' }, materials: { primary: 'iridescent_fabric' },
      condition: { items: [condItem('rain_soaked', { placements: ['shoes'] })] } });
    var d = G.detailed(o) + ' ' + G.short(o);
    assert(d.indexOf('shoes') < 0, '人魚に靴の状態が出る：' + d);
    // 値は保持される（カテゴリを戻せば復元）
    eq(o.condition.items[0].placements[0], 'shoes');
  });
  test('5B監査固定：人魚以外に尾の部位が入っても出力しない', function () {
    var o = condOutfit({ garment: { category: 'dress', subtype: 'shirt_dress' }, materials: { primary: 'cotton' },
      condition: { items: [condItem('scratched', { placements: ['mermaid_tail'] })] } });
    var d = G.detailed(o) + ' ' + G.short(o);
    assert(d.indexOf('mermaid tail') < 0, 'ドレスに人魚の尾の部位が出る：' + d);
  });

    /* ---- 競合・警告 ---- */
  test('5B後半：破れ＋金属で警告が出る', function () {
    var o = condOutfit({ garment: { category: 'uniform', subtype: 'royal_uniform' }, materials: { primary: 'plate_armor' },
      condition: { items: [condItem('torn_light')] } });
    var hit = AD.check(o).filter(function (i) { return i.id === 'cond_tear_on_metal'; });
    eq(hit.length, 1);
    eq(hit[0].severity, 'warning');
    assert(/scratched/.test(hit[0].messageJa), '代替候補の提示が無い');
  });
  test('5B後半：しわ＋金属で警告が出る', function () {
    var o = condOutfit({ garment: { category: 'uniform', subtype: 'royal_uniform' }, materials: { primary: 'plate_armor' },
      condition: { items: [condItem('wrinkled')] } });
    assert(AD.check(o).some(function (i) { return i.id === 'cond_wrinkle_on_metal'; }));
  });
  test('5B後半：水吸い＋防水で情報表示が出る', function () {
    var o = condOutfit({ garment: { category: 'top_bottom', subtype: 'hoodie_and_cargo' }, materials: { primary: 'nylon' },
      condition: { items: [condItem('water_soaked')] } });
    var hit = AD.check(o).filter(function (i) { return i.id === 'cond_soak_on_waterproof'; });
    eq(hit.length, 1);
    eq(hit[0].severity, 'info');
  });
  test('5B後半：毛玉＋金属はhard', function () {
    var o = condOutfit({ garment: { category: 'uniform', subtype: 'royal_uniform' }, materials: { primary: 'plate_armor' },
      condition: { items: [condItem('pilled')] } });
    assert(AD.check(o).some(function (i) { return i.id === 'cond_pile_on_metal' && i.severity === 'hard'; }));
  });
  test('5B後半：意味重複（加工×状態）の警告が出る', function () {
    var o = condOutfit({ garment: { category: 'top_bottom', subtype: 'simple_tshirt_and_jeans' }, materials: { primary: 'denim' },
      condition: { items: [{ type: 'distressed', group: 'treatment', severity: 'moderate', extent: 'localized', placements: [] }, condItem('tattered')] } });
    assert(AD.check(o).some(function (i) { return i.id === 'cond_distress_plus_heavy_tear'; }));
    var o2 = condOutfit({ garment: { category: 'top_bottom', subtype: 'simple_tshirt_and_jeans' }, materials: { primary: 'cotton' },
      condition: { items: [condItem('water_droplets'), condItem('drenched')] } });
    assert(AD.check(o2).some(function (i) { return i.id === 'cond_droplets_plus_drench' && i.severity === 'info'; }));
  });
  test('5B後半：布素材の泥はねでは金属系警告が出ない', function () {
    var o = condOutfit({ garment: { category: 'dress', subtype: 'shirt_dress' }, materials: { primary: 'cotton' },
      condition: { items: [condItem('mud_splattered')] } });
    assert(!AD.check(o).some(function (i) { return /^cond_/.test(i.id); }), '不要な状態警告');
  });

  /* ---- 補助候補 ---- */
  test('5B後半：冒険者服に状態候補が提案される', function () {
    var o = condOutfit({ concept: { worldview: 'western_fantasy' },
      garment: { category: 'top_bottom', subtype: 'adventurer_outfit' }, materials: { primary: 'canvas' } });
    var got = AD.suggest(o, { limit: 40 }).filter(function (c) { return c.category === '状態・加工'; });
    assert(got.length >= 1, '状態候補が無い');
    assert(got.every(function (c) { return U.byId(D.conditions, c.valueId); }));
  });
  test('5B後半：補助候補は採用するまで設計を変えない', function () {
    var o = condOutfit({ concept: { worldview: 'western_fantasy' },
      garment: { category: 'top_bottom', subtype: 'adventurer_outfit' }, materials: { primary: 'canvas' } });
    var before = JSON.stringify(o.condition);
    AD.suggest(o, { limit: 40 });
    eq(JSON.stringify(o.condition), before);
  });
  test('5B後半：夢かわルームウェアには状態候補も血も出さない', function () {
    ['dreamy_pastel_pajamas', 'pastel_fluffy_loungewear'].forEach(function (sub) {
      var o = condOutfit({ concept: { worldview: 'modern' },
        garment: { category: 'top_bottom', subtype: sub }, materials: { primary: 'plush_fabric' } });
      var got = AD.suggest(o, { limit: 40 }).filter(function (c) { return c.category === '状態・加工'; });
      eq(got.length, 0, sub + ' に状態候補が出る');
    });
  });
  test('5B後半：状態2件のときは追加候補を出さない', function () {
    var o = condOutfit({ concept: { worldview: 'western_fantasy' },
      garment: { category: 'top_bottom', subtype: 'adventurer_outfit' }, materials: { primary: 'canvas' },
      condition: { items: [condItem('worn'), condItem('mud_splattered')] } });
    var got = AD.suggest(o, { limit: 40 }).filter(function (c) { return c.category === '状態・加工'; });
    eq(got.length, 0);
  });
  test('5B後半：人魚には鱗・水系の候補が出る', function () {
    var o = condOutfit({ garment: { category: 'merfolk', subtype: 'mermaid_tail' },
      parts: { mermaid_tail_form: 'classic_scaled_tail' }, materials: { primary: 'iridescent_fabric' } });
    var got = AD.suggest(o, { limit: 40 }).filter(function (c) { return c.category === '状態・加工'; });
    assert(got.length >= 1, '人魚に状態候補が無い');
    got.forEach(function (c) {
      assert((D.conditionAffinity.byCategory.merfolk).indexOf(c.valueId) >= 0, '想定外の候補：' + c.valueId);
    });
  });

  /* ---- ガチャ ---- */
  test('5B後半：ガチャ対象に「状態・加工」がある', function () {
    assert(U.byId(GA.TARGETS, 'condition'), 'condition対象が無い');
  });
  test('5B後半：状態ガチャは採用まで元設計を変えない', function () {
    var o = condOutfit({ concept: { worldview: 'western_fantasy' },
      garment: { category: 'top_bottom', subtype: 'adventurer_outfit' }, materials: { primary: 'canvas' } });
    var before = JSON.stringify(o);
    GA.roll(o, { target: 'condition', seed: 7 });
    eq(JSON.stringify(o), before);
  });
  test('5B後半：状態ガチャは3件以上を生成しない', function () {
    var o = condOutfit({ concept: { worldview: 'western_fantasy' },
      garment: { category: 'top_bottom', subtype: 'adventurer_outfit' }, materials: { primary: 'canvas' } });
    for (var seed = 1; seed <= 30; seed++) {
      GA.roll(o, { target: 'condition', seed: seed }).forEach(function (r) {
        assert(r.preview.condition.items.length <= 2, '3件以上：seed=' + seed);
      });
    }
  });
  test('5B後半：金属素材の状態ガチャに破れ・しわが出ない', function () {
    var o = condOutfit({ garment: { category: 'uniform', subtype: 'royal_uniform' }, materials: { primary: 'plate_armor' } });
    for (var seed = 1; seed <= 30; seed++) {
      GA.roll(o, { target: 'condition', seed: seed }).forEach(function (r) {
        r.preview.condition.items.forEach(function (it) {
          var def = U.byId(D.conditions, it.type);
          assert(!((def.avoidClasses || []).indexOf('metallic') >= 0), '金属に不適合な状態：' + it.type);
        });
      });
    }
  });
  test('5B後半：人魚の状態ガチャは専用プールで脚系部位を出さない', function () {
    var o = condOutfit({ garment: { category: 'merfolk', subtype: 'mermaid_tail' },
      parts: { mermaid_tail_form: 'classic_scaled_tail' }, materials: { primary: 'iridescent_fabric' } });
    for (var seed = 1; seed <= 40; seed++) {
      GA.roll(o, { target: 'condition', seed: seed }).forEach(function (r) {
        r.preview.condition.items.forEach(function (it) {
          assert(D.conditionAffinity.merfolkGachaPool.indexOf(it.type) >= 0, '人魚プール外：' + it.type);
          (it.placements || []).forEach(function (pl) {
            var p = U.byId(D.conditionPlacements, pl);
            assert(!p.legRelated, '人魚に脚系部位：' + pl);
          });
        });
      });
    }
  });
  test('5B後半：他対象のガチャは血を含むconditionへ触れない', function () {
    var o = condOutfit({ concept: { worldview: 'modern' },
      garment: { category: 'top_bottom', subtype: 'simple_tshirt_and_jeans' }, materials: { primary: 'cotton' } });
    ['garment', 'materials', 'decorations', 'palette'].forEach(function (t) {
      for (var seed = 1; seed <= 10; seed++) {
        GA.roll(o, { target: t, seed: seed }).forEach(function (r) {
          eq(r.preview.condition.items.length, 0, t + ' が状態へ触れた');
        });
      }
    });
  });
  test('5B後半：夢かわ系の状態ガチャに血が出ない', function () {
    var o = condOutfit({ concept: { worldview: 'modern' },
      garment: { category: 'top_bottom', subtype: 'dreamy_pastel_pajamas' }, materials: { primary: 'plush_fabric' } });
    for (var seed = 1; seed <= 60; seed++) {
      GA.roll(o, { target: 'condition', seed: seed }).forEach(function (r) {
        r.preview.condition.items.forEach(function (it) {
          var def = U.byId(D.conditions, it.type);
          assert(!(def.tags && def.tags.indexOf('cond_blood') >= 0), '夢かわに血：seed=' + seed);
        });
      });
    }
  });
  test('5B後半：状態ガチャの案の差分ラベルが日本語で出る', function () {
    var o = condOutfit({ concept: { worldview: 'western_fantasy' },
      garment: { category: 'top_bottom', subtype: 'adventurer_outfit' }, materials: { primary: 'canvas' } });
    var found = false;
    for (var seed = 1; seed <= 30 && !found; seed++) {
      GA.roll(o, { target: 'condition', seed: seed }).forEach(function (r) {
        if (!r.preview.condition.items.length) return;
        found = true;
        r.diff.forEach(function (dd) {
          assert(dd.labelJa === '状態・加工', 'ラベル不正：' + dd.labelJa);
          assert(dd.toJa.indexOf('undefined') < 0, dd.toJa);
        });
      });
    }
    assert(found, '状態付きの案が一度も出ない');
  });

  /* ---- 状態付きプリセットの生成確認 ---- */
  test('5B後半：状態付きプリセット8件がすべて生成に通る', function () {
    D.presets.filter(function (pr) { return pr.group === 'scene'; }).forEach(function (pr) {
      var o = SC.applyPatch(SC.createOutfit(), pr.patch);
      var d = G.detailed(o);
      var sBad = /undefined|null|,\s*,|\.\s*\.|with wet|with dirt|torn open/;
      assert(!sBad.test(d), pr.id + '：' + d);
      assert(o.condition.items.length >= 1 && o.condition.items.length <= 2, pr.id + ' の状態件数');
    });
  });
  test('5B後半：血痕付き白衣プリセットに負傷・goreが出ない', function () {
    var pr = U.byId(D.presets, 'bloodstained_labcoat');
    var o = SC.applyPatch(SC.createOutfit(), pr.patch);
    var d = G.detailed(o) + ' ' + G.short(o);
    assert(/blood-stained/.test(d), d);
    assert(!/wound|injur|bleeding|gore/i.test(d), d);
  });



  /* ---- ChatGPT 最終独立監査：英文回帰 ---- */
  test('最終監査：人魚では人間衣装由来の下半身ボリューム・丈を出さない', function () {
    var o = merOutfit();
    o.silhouette.lowerVolume = 'voluminous';
    o.silhouette.length = 'knee';
    var out = G.short(o) + ' || ' + G.detailed(o);
    assert(!/voluminous skirt|slim lower body|flared|micro length|knee-length|midi length|floor-length|trailing hem/i.test(out), out);
    assert(/single fish tail|single classic scaled mermaid tail/.test(out), out);
  });

  test('最終監査：着用役割は動詞句のまま接続し featuring を重ねない', function () {
    ['inner_layer', 'visible_inner', 'lingerie_fashion', 'layered_with_outerwear'].forEach(function (role) {
      var o = condOutfit({
        garment: { category: 'dress', subtype: 'ball_gown', wearRole: role },
        silhouette: { fit: 'fitted' },
        materials: { primary: 'silk' },
        palette: { primary: 'deep_navy' }
      });
      var d = G.detailed(o);
      var def = U.byId(D.wearRoles, role);
      assert(d.indexOf(def.promptEn) >= 0, role + ' が欠落：' + d);
      assert(!/featuring\s+(worn|styled|layered)/i.test(d), role + ' の接続が不正：' + d);
    });
  });

  test('最終監査：ランジェリー型の詳細版は冠詞付きの完全な名詞句になる', function () {
    var two = condOutfit({
      garment: { category: 'lingerie', subtype: 'lace_bodysuit', wearRole: 'main_outfit' },
      silhouette: { fit: 'fitted' }, parts: { lingerie_form: 'separate_set' },
      materials: { primary: 'lace' }, palette: { primary: 'jet_black' }
    });
    var one = SC.applyPatch(two, { parts: { lingerie_form: 'one_piece_lingerie' } });
    var d2 = G.detailed(two), d1 = G.detailed(one);
    assert(/with a two-piece set/.test(d2), d2);
    assert(/with a one-piece design/.test(d1), d1);
    assert(!/with two-piece set|with one-piece(?:[,.]|$)/.test(d2 + ' ' + d1), d2 + ' || ' + d1);
  });

  test('最終監査：一点の可算装飾は単数形になる', function () {
    var cases = {
      ribbon_bow: 'ribbon bow', rose_ornament: 'rose ornament', metal_studs: 'metal stud',
      medals: 'medal', aiguillettes: 'aiguillette', gem_settings: 'gem setting'
    };
    Object.keys(cases).forEach(function (id) {
      var o = condOutfit({
        garment: { category: 'dress', subtype: 'ball_gown' }, silhouette: { fit: 'fitted' },
        materials: { primary: 'silk' }, palette: { primary: 'deep_navy' },
        decorations: { density: 2, items: [{ type: id, placements: ['chest'], role: 'support', size: 'medium', quantity: 'single' }] }
      });
      var d = G.detailed(o);
      assert(d.indexOf('a single ' + cases[id]) >= 0, id + '：' + d);
      assert(!new RegExp('a single ' + cases[id].replace(/[.*+?^${}()|[\\]\\]/g, '\\$&') + 's\\b').test(d), id + ' が複数形：' + d);
    });
  });

  test('最終監査：複数の可算装飾は複数形になる', function () {
    [['sapphire_brooch', 'a few sapphire brooches'], ['ribbon_bow', 'numerous ribbon bows'], ['generic_badges', 'a few badges']].forEach(function (c) {
      var qty = c[1].indexOf('numerous') === 0 ? 'many' : 'few';
      var o = condOutfit({
        garment: { category: 'dress', subtype: 'ball_gown' }, silhouette: { fit: 'fitted' },
        materials: { primary: 'silk' }, palette: { primary: 'deep_navy' },
        decorations: { density: 2, items: [{ type: c[0], placements: ['chest'], role: 'support', size: 'medium', quantity: qty }] }
      });
      var d = G.detailed(o);
      assert(d.indexOf(c[1]) >= 0, c[0] + '：' + d);
    });
  });

  test('最終監査：特殊パーツの詳細版は動詞接続を二重にしない', function () {
    var horn = outfit({ garment: { category: 'dress', subtype: 'ball_gown' }, specialParts: { horns: { type: 'horn_curved' } } });
    var halo = outfit({ garment: { category: 'dress', subtype: 'ball_gown' }, specialParts: { halo: { type: 'halo_ring' } } });
    var wing = outfit({ garment: { category: 'dress', subtype: 'ball_gown' }, specialParts: { wings: { type: 'angel_wings', count: 'wing_one_pair' } } });
    var all = [G.detailed(horn), G.detailed(halo), G.detailed(wing)].join(' || ');
    assert(/featuring curved horns/.test(G.detailed(horn)), G.detailed(horn));
    assert(/crowned by a halo/.test(G.detailed(halo)), G.detailed(halo));
    assert(/completed with a pair of angel wings behind the wearer/.test(G.detailed(wing)), G.detailed(wing));
    assert(!/completed with\s+(featuring|crowned by|completed with)|with completed with/i.test(all), all);
  });

  test('最終監査：宝石色の詳細版で highlights を重複させない', function () {
    var o = condOutfit({
      garment: { category: 'uniform', subtype: 'business_suit' }, silhouette: { fit: 'tailored' },
      materials: { primary: 'wool' }, palette: { primary: 'charcoal_gray', accent: 'sapphire_blue' }
    });
    var d = G.detailed(o);
    assert(/(?:highlighted with|accented with) sapphire blue/.test(d), d);
    assert(!/sapphire blue highlights|highlights highlights/.test(d), d);
  });
