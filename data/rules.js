...
(function (global) {
  ...
  CPW.data.rules = [
    ...
    /* ---------- 状態・加工（Phase 5B 後半） ---------- */
    {
      id: 'cond_tear_on_metal',
      kind: 'pair', type: 'physical', severity: 'warning', category: '状態・加工',
      left: { paths: ['condition.items'], tags: ['cond_tear'] },
      right: { paths: ['materials.primary'], tags: ['mat_metallic', 'mat_rigid'] },
      titleJa: '金属・硬質素材に布の破れ',
      messageJa: '金属や硬質素材は布のように破れません。出力では scratched / dented / chipped など硬質素材向けの傷表現へ自動で言い換えますが、種類を「傷」「へこみ」「欠け」へ変えると意図がより正確に伝わります。',
      resolutions: [
        { labelJa: 'このまま維持する（出力側で言い換え）', action: 'ignore' }
      ]
    },
    {
      id: 'cond_wrinkle_on_metal',
      kind: 'pair', type: 'physical', severity: 'warning', category: '状態・加工',
      left: { paths: ['condition.items'], tags: ['cond_wrinkle'] },
      right: { paths: ['materials.primary'], tags: ['mat_metallic', 'mat_rigid'] },
      titleJa: '金属・硬質素材にしわ・くたびれ',
      messageJa: '金属や硬質素材にはしわが寄りません。出力では dented / scuffed へ自動で言い換えます。「へこみ」「擦れ」を選び直すのも一案です。',
      resolutions: [
        { labelJa: 'このまま維持する（出力側で言い換え）', action: 'ignore' }
      ]
    },
    {
      id: 'cond_soak_on_waterproof',
      kind: 'pair', type: 'physical', severity: 'info', category: '状態・加工',
      left: { paths: ['condition.items'], tags: ['cond_wet_absorb'] },
      right: { paths: ['materials.primary'], tags: ['mat_waterproof'] },
      titleJa: '防水素材に「水を吸った」表現',
      messageJa: '防水素材は水を吸いません。出力では水滴（covered in water droplets / rain-beaded）系の表現へ自動で置き換えます。「水滴が付いている」を選ぶと表示と出力が一致します。',
      resolutions: [
        { labelJa: 'このまま維持する（出力側で言い換え）', action: 'ignore' }
      ]
    },
    {
      id: 'cond_pile_on_metal',
      kind: 'pair', type: 'physical', severity: 'hard', category: '状態・加工',
      left: { paths: ['condition.items'], tags: ['cond_pile'] },
      right: { paths: ['materials.primary'], tags: ['mat_metallic'] },
      titleJa: '金属素材に毛玉・毛羽立ち',
      messageJa: '毛玉・毛羽立ちは繊維の状態で、金属素材とは両立しません。素材か状態のどちらかを変えてください。',
      resolutions: [
        { labelJa: 'このまま維持する', action: 'ignore' }
      ]
    },
    {
      id: 'cond_distress_plus_heavy_tear',
      kind: 'pair', type: 'semantic', severity: 'warning', category: '状態・加工',
      left: { paths: ['condition.items'], tags: ['cond_treat_damage'] },
      right: { paths: ['condition.items'], tags: ['cond_heavy_tear'] },
      titleJa: 'ダメージ加工と激しい破れの重複',
      messageJa: '「最初からのダメージ加工」と「激しい破れ」は意味が重なります。デザインとしての加工なら加工だけ、実際に傷んだ状態なら破れだけにすると、画像生成が安定します。',
      resolutions: [
        { labelJa: 'このまま維持する', action: 'ignore' }
      ]
    },
    {
      id: 'cond_repair_duplicate',
      kind: 'pair', type: 'semantic', severity: 'warning', category: '状態・加工',
      left: { paths: ['condition.items'], tags: ['cond_treat_repair'] },
      right: { paths: ['condition.items'], tags: ['cond_repair'] },
      titleJa: '補修加工と補修済みの重複',
      messageJa: '補修風ステッチ（加工）と補修跡・補修済み（状態）は意味が重なります。どちらか一方で十分です。',
      resolutions: [
        { labelJa: 'このまま維持する', action: 'ignore' }
      ]
    },
    {
      id: 'cond_fade_duplicate',
      kind: 'pair', type: 'semantic', severity: 'warning', category: '状態・加工',
      left: { paths: ['condition.items'], tags: ['cond_treat_fade'] },
      right: { paths: ['condition.items'], tags: ['cond_fade'] },
      titleJa: '色落ち加工と色褪せの重複',
      messageJa: '色落ち加工（デザイン）と色褪せ（経年）は見た目が重なります。どちらの由来かを一つに絞ると、意図が伝わりやすくなります。',
      resolutions: [
        { labelJa: 'このまま維持する', action: 'ignore' }
      ]
    },
    {
      id: 'cond_droplets_plus_drench',
      kind: 'pair', type: 'semantic', severity: 'info', category: '状態・加工',
      left: { paths: ['condition.items'], tags: ['cond_droplets'] },
      right: { paths: ['condition.items'], tags: ['cond_drench'] },
      titleJa: '水滴とずぶ濡れの重複',
      messageJa: '水滴が付いた状態とずぶ濡れは段階が違う濡れ方です。両方指定すると、強い方（ずぶ濡れ）に引っ張られやすくなります。',
      resolutions: [
        { labelJa: 'このまま維持する', action: 'ignore' }
      ]
    }
