...
`version` は `CPW.SCHEMA_VERSION`（現在 **0.4**）。古い版のデータは読み込み時に移行する（部位スロットの旧ID、特殊パーツの旧形式、0.1／0.2 の `condition` 欠落は空配列で補完）。0.1・0.2・0.3 のデータはそのまま読める。旧データには空の `styling` も補完する。

## 衣装の状態・加工（Phase 5B 後半）

汚れ・破れ・濡れ・血の付着・経年・意図的な加工を、素材・装飾とは別の独立レイヤーとして指定する。設計台では「素材・装飾」の次に独立セクション「衣装の状態・加工」がある。

保存構造（現行スキーマ0.4、conditionは0.3から互換）：
...
- 競合ルールは44件（Phase 5Cで仕立て・着こなしのwarning 6件を追加）。収録データが増えれば取りこぼしも増える。
