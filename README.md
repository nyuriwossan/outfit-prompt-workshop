...
開始画面の **「細かく選んで作る」／「コンセプトから作る」** を維持し、`#/entry`には通常の5開始方法だけを表示します。コンセプト画面は次の5ステップです。

1. 題材：主題、任意の副題、混ぜ方
2. 世界・人物：世界観、時代、役割、場面、物語状態（最大2）
3. デザイン：方向性（最大2）、造形言語（最大2）、解釈方法
4. 衣装：衣装ベース、既存を残す／新規
5. 反映：反映先（最大4）、強さ、被覆バランス、除外（最大4）

主題の保存フィールド `categoryId / motifId / customMotif` はそのままです。副題は追加ボタンで開き、削除すると空に戻ります。混ぜ方は主題中心・半々・役割分担・副題をアクセントの4種類で、配色／素材／仕立て／装飾の割り当てを変えます。同じ題材の二重指定は警告し、二重に適用しません。

世界・人物には既存の `data.worldviews / eras / roles / occasions` を再使用しています。候補では既存conceptフィールドへ反映し、採用後の設計台編集を正本として扱います。物語状態は既存conditionとstylingへ翻訳し、それぞれ最大2、既存の適用判定を通します。戦闘後でも血液は自動追加しません。

方向性は既存の様式へ、造形言語はsilhouetteとpartSlotsへ明示的にマッピングします。解釈はauto／literal／symbolic／abstract／fashion／stage／avant_garde。薔薇の花飾りをプリーツのリズムへ、歯車を金具の反復へ変えるなど、装飾の抽象化にも元のモチーフとの対応を持たせています。未知の自由入力から意味を推測せず、customTagsには書き込みません。

3案は `readable`（分かりやすく）、`wearable`（自然な服として）、`experimental`（大胆に再構成）。形・素材の組み合わせ・仕立て・装飾密度を比較できます。`motifContributions`に主題／副題が反映された構造化パスを記録し、理由と警告を分けて表示します。反映先が配色だけの場合や、preserve・伝統衣装で構造を固定した場合には差が限定される理由を表示します。

除外は共通の `constraintViolations / enforceExclusions` で扱う絶対条件です。新規候補が制約を満たせない場合は候補を出さず、見直す項目を表示します。preserveで既存値と競合する場合はプレビューで勝手に削除せず、採用時に「既存を残す」「除外条件を優先して外す」を選びます。残す場合は競合する除外条件を解除して保存します。基本衣装そのものに含まれる特徴を安全に除去できない場合は、採用せず衣装ベースの見直しを案内します。

`CPW.conceptFashion.randomSelection(profileId, seed)` は11種のおまかせ傾向を持ち、同じseedで同じ選択を再現します。伝統衣装・食品の実物配置・極端な被覆・血液は無差別選択しません。人物性別を推測したり、性別で衣装を制限したりしません。
...
モチーフ185件を追加。食品72・伝統衣装14は維持し、非食品を拡張しました。カテゴリ別は概念30、幻想28、物語29、食品72、自然32、生物30、素材28、美術27、伝統14、舞台21。主要な語彙追加394件、混ぜ方4・戦略3・おまかせ傾向11を含めると追加412件です。色128・素材50・柄30・styling34・condition53も維持しています。

### 保存・互換と英文

schemaは **0.6**。0.1〜0.5を読み込み、旧0.5の主題は維持したままsecondary／context／designLanguages／exclusions等の空フィールドを補完します。0.3・0.4の各32件、およびmain由来0.5の144件について、移行前後の短縮版・詳細版を完全一致で検証しています。保存・再読込・複製・単体JSON・ライブラリJSON・customTagsも検証対象です。

generatorは置き換えず、構造化済みモチーフの識別句を重複追加しない処理だけを拡張しました。物語状態の長文や除外条件のnegative promptを追加しません。merfolk・素材別condition表現・styling適用判定・特殊パーツ必須軸・presentation・既存advisor/rulesを使用します。追加の5警告は重複主題、物語状態、除外競合、時代ミックス、役割と場面を知らせるもので、意図的な創作ミックスをhard禁止しません。

### 検証と再実行

```sh
node scripts/test.cjs
node scripts/counts-phase5e.cjs
node scripts/audit-phase5e.cjs 250000 verification/phase5e-audit-250k.json
node scripts/ui-phase5e.cjs verification/phase5e-ui.json
node scripts/ui-phase5d.cjs verification/phase5e-legacy-ui.json
node scripts/ui.cjs
```

ブラウザ検証だけ開発用PlaywrightとEdgeを使用します。配布アプリにnpm・ビルド・外部APIは不要です。CP0→CP9の検証記録は`verification/phase5e-*`。既存809件を削除せず、schema更新と語彙増量に必要な期待値だけを更新しています。最新の全件結果・監査結果・UI結果は同フォルダのJSONを参照してください。

最終結果は **1,386 / 1,386通過**（既存809＋追加577）。25万衣装ケースから短縮版・詳細版の計50万英文を生成し、監査の検出問題は0件でした。375／390／430pxの各幅で、新旧UIの回帰、横はみ出しなし、44px操作対象、採用前state不変、保存・再読込、従属selectと自動refocusなしを確認しています。

制約を固定しないおまかせ1,000選択では、全1,000組が2領域以上で異なり、3,000ペアに同一候補はありません。全体監査の62,500組では51,107組が2領域以上で異なり、11,393組は既存値の保持・反映先・食品用途・伝統構造などの制約による差分制限を記録しました。監査JSONの`identical`は制約付きケースの同一ペア19,631件、`colorOnly`は素材領域だけが異なるペア6,099件を表します。無制約の候補がすべて異なるという結果を、制約付きの全ケースへ一般化していません。

### 既知制約とiPhone Safari実機確認

自由入力は自動翻訳・意味推論を行いません。副題の一覧はモチーフで、伝統衣装は主題の専用UIから指定します。文化的衣装や年代の完全な史実再現は保証しません。厳しい除外や反映先指定、preserveによって候補の構造差が小さくなる場合は、その理由を表示します。

自動UI検証はWindows Edgeの375／390／430pxとタッチエミュレーションです。iPhone SafariのネイティブUI、Android実機を確認したという意味ではありません。select変更後の自動focusは追加していません。iPhone Safariでは最後に以下を実機確認してください。

1. 主題・カテゴリのselectが1回で閉じ、値が保持される
2. 副題・従属selectも二重表示しない
3. 5ステップナビが横にはみ出さない
4. 除外の選択・解除をタップしやすい
5. 3候補が縦に読みやすく、理由を開閉できる
6. 下部ナビと候補採用ボタンが重ならない
7. ノッチ・ホームバーのsafe-area内に操作要素が収まる
8. ブラウザ再起動後も保存した衣装と複合コンセプトが復元される
...
| 5E | 複合モチーフ・世界人物・物語・除外・3案戦略、保存0.6 | 実装済み |
| 5F | Inspired題材471件・題材検索・かんたん設計／こだわり設計、保存0.6を維持 | 実装済み |
...
- 競合ルール・infoは50件（Phase 5Cのwarning 6件とPhase 5Dの文化的衣装info 1件を含む）。収録データが増えれば取りこぼしも増える。
...
- 収録数：色128、素材50（全件 matClasses 付き）、柄30、装飾81、様式54、着こなし34、通常設計モチーフ39、基本衣装94、プリセット32（うち状態付き8）、状態・加工53種（部位18種）。
...
## Phase 5F：入口は簡単、奥は底なし

Inspired題材を **311→471件（160件追加）** に拡張しました。「物品・道具」35件を追加し、砂時計・香水瓶・懐中時計・羅針盤・鳥籠・天球儀などを色、素材、輪郭、仕立て、装飾へ翻訳します。自然・生物・宝石・工芸も拡張し、オパール、モルフォ蝶、海溝、バラ窓などを追加しました。既存のステンドグラス・クラゲ・孔雀等は同じIDを使います。

| カテゴリ | Phase 5E | Phase 5F |
|---|---:|---:|
| 概念・感情 | 30 | 40 |
| ファンタジー・超常 | 28 | 40 |
| 物語・神話・伝承 | 29 | 40 |
...
カテゴリ別のおすすめから選ぶほか、日本語・英語・別名で部分一致検索できます。カテゴリ内検索と「すべてから検索」を切り替えられ、結果は最大30件です。検索は端末内だけで処理します。

通常設計とコンセプト作成には共通の「かんたん設計／こだわり設計」を追加しました。初期表示はかんたん設計です。通常設計では主要コンセプト、衣装・基本部位、素材・柄・装飾密度、主色・副色・差し色を表示し、人魚の尾など必須部位は常に扱えます。詳細な着こなし・状態・特殊パーツへは「もっと細かく仕上げる」から進めます。こだわり設計には従来の全7セクションを残しています。

コンセプトのかんたん設計は **題材 → 雰囲気・衣装 → 仕上げ** の3ステップ、こだわり設計は従来の5ステップです。切替だけでは編集中の選択・候補・衣装データ・dirty・更新日時・衣装の自動保存を変更しません。非表示でも指定済みの値は有効で、件数・グループ・副題を要約します。

表示設定は別キー `cpw.uiPrefs.v1` の `workshopLevel` / `conceptLevel` に保存します。両画面の設定は独立し、保存が禁止された環境ではメモリで保持します。**schema 0.6、0.1〜0.5の移行、JSON、customTagsは従来どおり**です。

検証結果：既存1,524件を保持し、**1,956/1,956テスト通過（432件追加）**。新規160件のmappingは100%が2軸以上。新規題材中心の5,000 selectionで構造差を確認し、100,002英文の監査で未解決の問題はありません。375/390/430pxのChromium・WebKitで両表示レベル、採用、再読み込み、非表示値、配色ピッカーを確認しました。ミルフィーユの案A/B/C、new/preserve、keep/exclude/cancel、native confirmに依存しない確認、monochrome抑止、selectへの自動再focus禁止も維持しています。

```sh
node scripts/test.cjs
node scripts/audit-phase5f.cjs verification/phase5f-audit.json
# 以下のみ開発用Playwrightとブラウザが必要です。本体に依存関係はありません。
node scripts/ui-phase5f.cjs
node scripts/ui-phase5f-details.cjs
node scripts/ui-phase5e-hotfix.cjs
node scripts/ui-phase5e-confirm.cjs
```

UIスクリプトは既定でEdge（Chromium）を使います。環境変数 `CPW_ENGINE=webkit` でWebKitも確認できます。結果は `verification/phase5f-*`、最終報告27項目は [PHASE5F-REPORT.md](PHASE5F-REPORT.md) に記録しています。WebKit自動化はiPhone実機そのものではないため、Safariのネイティブ選択UI・ソフトキーボード・セーフエリアは最後に実機で確認してください。
