...
(function (global) {
  ...
  function garmentHead(o) {
    ...
    if (!c) return null;
    return { top_bottom: 'outfit', dress: 'dress', uniform: 'uniform', robe: 'robe', wafuku: 'kimono', chinese: 'hanfu', swimwear: 'swimsuit', lingerie: 'lingerie set', merfolk: 'mermaid outfit' }[c.id] || 'outfit';
  }
  ...
  /* 可算装飾の単複を数量に合わせる。データの shortPrompt は、
   * 単数形・複数形のどちらで登録されていてもよい。 */
  function singularizeLastWord(phrase) {
    return phrase.replace(/([A-Za-z-]+)$/, function (word) {
  ...
  function pluralizeLastWord(phrase) {
    return phrase.replace(/([A-Za-z-]+)$/, function (word) {
      ...
      if (/(ch|sh|x|z|ss)$/i.test(word)) return word + 'es';
      return word + 's';
    });
  }
  ...
  function decorationPhrase(item, opts) {
    ...
    if (opts && opts.detailed) {
      ...
      var qtyWord = null;
      if (qty && !d.uncountable) {
        name = qty.id === 'single' ? singularizeLastWord(name) : pluralizeLastWord(name);
        qtyWord = qty.shortPrompt;
      }
  ...
  function blockSilhouette(o) {
    var s = o.silhouette;
    var isMerfolk = o.garment.category === 'merfolk';
    var list = [
  ...
  function blockParts(o, headWords) {
    ...
    cat.slots.forEach(function (slotId) {
      ...
      if (kind === 'composite') {
        ...
        if (p) { out.push(p); detailOut.push(p); }
        return;
      }
      if (kind === 'multi') {
  ...
  /* チェックポイント4：人魚の必須句。
   * 人魚カテゴリのときだけ、「一本の魚の尾で、人間の脚は無い」ことを明示する。
   * 画像AIは mermaid tail だけだと脚＋尾を描きがちなので、否定句を必ず添える。
   * 短縮版の 'single fish tail' は 'mermaid tail' と語が重ならず、compress に落とされない。 */
  function blockMerfolk(o) {
    if (o.garment.category !== 'merfolk') return { short: [], detailed: [] };
    // 尾の形（mermaid_tail_form）はここで一文に統合する。
    // blockParts と二重に出すと "with classic scaled mermaid tail" と
    // "single scaled mermaid tail" が重複するため（仕様書§10）。
    var slot = U.byId(D.partSlots, 'mermaid_tail_form');
    var form = slot ? en(slot.options, o.parts.mermaid_tail_form) : null;
    var tail = form || 'scaled mermaid tail';
    var sh = ['single fish tail', 'no human legs', 'no feet'];
    if (form) sh.unshift(form);
    return {
      short: sh,
      detailed: ['with the lower body ending in a single ' + tail + ', replacing separate human legs and feet']
    };
  }
   ...
   * ・素材の matClasses に応じて言い換える（金属は破れない・防水は吸わない）。
   * ・濡れから透け・密着・露出を、血から負傷・gore を、絶対に足さない。
   * ・空なら接続語ごと出さない。'with wet' / 'with dirt' / 'torn open' は作らない。
   * ========================================================== */
  var COND_CLASS_PRIORITY = ['scales', 'metallic', 'rigid', 'waterproof', 'fur', 'plush', 'knit', 'leather', 'textile'];

  function conditionMatClasses(o, item) {
    var m = U.byId(D.materials, o.materials.primary);
    var classes = (m && m.matClasses) ? m.matClasses.slice() : [];
    var def = U.byId(D.conditions, item.type);
    var pls = item.placements || [];
    var onTail = pls.indexOf('mermaid_tail') >= 0 || pls.indexOf('tail_fin') >= 0;
    if (o.garment.category === 'merfolk' && (onTail || (!pls.length && def && def.family === 'wet'))) {
      classes.unshift('scales');            // 人魚の尾・濡れは鱗の表現を優先（仕様書§7-7）
    }
    return classes;
  }

  /* 素材分類に合わせた基本句。分類不明なら base（安全な一般表現）へフォールバック。 */
  function conditionBase(def, classes) {
    if (def.byClass) {
      for (var i = 0; i < COND_CLASS_PRIORITY.length; i++) {
        var c = COND_CLASS_PRIORITY[i];
        if (classes.indexOf(c) >= 0 && def.byClass[c]) return def.byClass[c];
      }
    }
    return def.base;
  }

  function conditionPhrase(o, item, opts) {
    var def = U.byId(D.conditions, item.type);
    if (!def) return null;                             // 未知IDは静かに落とす（停止しない）
    var detailedMode = !(opts && opts.short);

    var classes = conditionMatClasses(o, item);
    var base = conditionBase(def, classes);

    // 程度。句が強さを含む型（caked with mud 等）には二重に付けない。
    var sev = U.byId(D.conditionSeverities, item.severity);
    var adverb = (!def.noAdverb && sev && sev.adverb) ? sev.adverb : '';
    var core = base;
    if (adverb) {
      // with句には with の内側へ入れる：'with chipped scales' → 'with lightly chipped scales'
      var m = base.match(/^with (a |the )?/);
      if (m) core = m[0] + adverb + ' ' + base.slice(m[0].length);
      else core = adverb + ' ' + base;
    }

    if (!detailedMode) {
      // 短縮版は簡潔なタグに畳む。先頭の with を外すだけで、句そのものは変えない。
      return core.replace(/^with (a |the )?/, '');
    }

    // 範囲・部位。'torn at overall' のような機械的な連結を作らない。
    var isMer = o.garment.category === 'merfolk';
    var pls = (item.placements || [])
      .map(function (id) { return U.byId(D.conditionPlacements, id); })
      .filter(Boolean)
      // 人魚では脚・靴系の部位を出力しない（値は保持し、カテゴリを戻せば復元される）。
      // 人魚以外では尾・尾びれの部位を出力しない。UI・ガチャに加えた、インポートJSON向けの防御。
      .filter(function (p) { return isMer ? !p.legRelated : !p.merfolkOnly; });
    var wholeGarment = pls.some(function (p) { return p.wholeGarment; });
    if (wholeGarment) pls = [];

    // 句がすでに鱗（scales）へ言及しているなら、「人魚の尾」の位置句は重ねない（同じ尾を二重説明しない）。
    if (words(base).indexOf('scales') >= 0) {
      pls = pls.filter(function (p) { return p.id !== 'mermaid_tail'; });
    }

    var nouns = joinAnd(pls.map(function (p) { return p.nounEn; }));
    var ats = pls.map(function (p) { return p.atEn; }).join(' and ');
    var loc = '';

    if (item.extent === 'overall' || wholeGarment) {
      loc = 'throughout';
    } else if (item.extent === 'widespread') {
      loc = 'across much of ' + (nouns || 'the garment');
    } else if (item.extent === 'scattered') {
      if (base.indexOf('in several places') >= 0) loc = nouns ? 'across ' + nouns : '';
      else loc = 'in several places' + (nouns ? ' across ' + nouns : '');
    } else {
      loc = ats;
    }
  ...
  function blockCondition(o) {
    var items = (o.condition && Array.isArray(o.condition.items)) ? o.condition.items : [];
    if (!items.length) return { short: [], detailed: [] };

    // 加工（treatment）はデザインなので先、状態（state）は後。並びは安定に保つ。
    var ordered = items.slice().sort(function (a, b) {
      var r = function (i) { return i.group === 'treatment' ? 0 : 1; };
      return r(a) - r(b);
    });

    var sh = ordered.map(function (i) { return conditionPhrase(o, i, { short: true }); }).filter(Boolean);
    var de = ordered.map(function (i) { return conditionPhrase(o, i); }).filter(Boolean);
    if (!de.length && !sh.length) return { short: [], detailed: [] };
    return { short: sh, detailed: de.length ? [joinAnd(de)] : [] };
  }
  ...
  function blockSpecialParts(o) {
    ...
    if (wings.length) sentences.push('completed with ' + joinAnd(wings) + ' behind the wearer');
    ...
    if (chains.length) sentences.push('detailed with ' + joinAnd(chains));
