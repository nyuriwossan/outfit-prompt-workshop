...
(function (global) {
  ...
  function tokens(outfit) {
    ...
    /* 部位。カテゴリの slots に無いスロットは収集しない。
     * 値は outfit.parts に保持されたままだが（カテゴリを戻せば復元される）、
     * 表示・警告の対象にはしない。人魚カテゴリ中の脚系スロットがこれに当たる。 */
    var catForSlots = U.byId(D.garmentCategories, o.garment.category);
    D.partSlots.forEach(function (slot) {
      if (catForSlots && catForSlots.slots.indexOf(slot.id) < 0) return;
    ...
    /* 素材。主素材には matClasses を mat_〜 タグとして持たせ、状態・加工との整合判定に使う。 */
    ['primary', 'secondary', 'trim'].forEach(function (k) {
      var mat = U.byId(D.materials, o.materials[k]);
      var extra = { kind: 'material' };
      if (k === 'primary' && mat) {
        extra.tags = (mat.tags || []).concat((mat.matClasses || []).map(function (c) { return 'mat_' + c; }));
      }
      add(tok('materials.' + k, o.materials[k], mat, extra));
    });
    ...
    /* 状態・加工（Phase 5B 後半） */
    ((o.condition && o.condition.items) || []).forEach(function (item, i) {
      if (!item.type) return;
      var def = U.byId(D.conditions, item.type);
      if (!def) return;
      add(tok('condition.items', item.type, def,
        { kind: 'condition', index: i,
          tags: (def.tags || []).concat(['cond', 'cond_' + def.family, 'cond_group_' + def.group]) }));
    });
  ...
  var CHECKS = {
    ...
    conceptDuplicate:function(o){return CPW.conceptFashion.checkNote?CPW.conceptFashion.checkNote(o,'duplicate'):null;},
    conceptStory:function(o){return CPW.conceptFashion.checkNote?CPW.conceptFashion.checkNote(o,'story'):null;},
    conceptExclusions:function(o){return CPW.conceptFashion.checkNote?CPW.conceptFashion.checkNote(o,'exclusions'):null;},
    conceptEra:function(o){return CPW.conceptFashion.checkNote?CPW.conceptFashion.checkNote(o,'era'):null;},
    conceptRole:function(o){return CPW.conceptFashion.checkNote?CPW.conceptFashion.checkNote(o,'role'):null;},
  ...
  /* 状態・加工の補助候補（Phase 5B 後半）。
   * ・提案するだけで、採用するまで設計へ反映しない（Phase 4 の約束のまま）。
   * ・夢かわ・ルームウェア系（noAutoSubtypes）は「状態指定なし」を基本とし、候補を出さない。
   * ・血の候補は、conditionAffinity に明示された衣装（白衣・マフィア風スーツ等）だけに限る。 */
  function conditionCandidates(o, ctx) {
    var out = [];
    var aff = D.conditionAffinity || {};
    var cur = (o.condition && o.condition.items) || [];
    if (cur.length >= 2) return out;                       // 上限に達していれば提案しない
    if ((aff.noAutoSubtypes || []).indexOf(o.garment.subtype) >= 0) return out;

    var ids = (aff.bySubtype && aff.bySubtype[o.garment.subtype]) ||
              (aff.byCategory && aff.byCategory[o.garment.category]) || [];
    ids.forEach(function (id) {
      var def = U.byId(D.conditions, id);
      if (!def) return;
      if (cur.some(function (it) { return it.type === id; })) return;
      var mat = U.byId(D.materials, o.materials.primary);
      var classes = (mat && mat.matClasses) || [];
      if ((def.avoidClasses || []).some(function (c) { return classes.indexOf(c) >= 0; })) return;

      var placements = [];
      if (def.tags && def.tags.indexOf('cond_tear') >= 0) placements = ['hem'];
      if (o.garment.category === 'merfolk' && def.family === 'wet') placements = ['mermaid_tail'];

      var item = { type: id, group: def.group, severity: 'light', extent: 'localized', placements: placements };
      out.push(cand(o, {
        kind: 'standard', category: '状態・加工', path: 'condition.items', valueId: id,
        labelJa: def.labelJa, promptEn: def.base, score: 4,
        reasons: ['この衣装と相性の良い状態・加工。軽度・一部から試して、程度と部位は後から調整できる'],
        patch: { condition: { items: cur.concat([item]) } }
      }));
    });
    return out;
  }
