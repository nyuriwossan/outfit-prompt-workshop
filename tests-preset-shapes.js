...
(function(g){
  ...
  ['short','detailed'].forEach(function(mode){
    test('backless knit '+mode,function(){var t=C.generator[mode](outfit('cutout_knit'));['virgin killer sweater','completely open back','ribbon tie at the nape','high neckline','sleeveless','opaque rib knit','matte finish','heather gray','jet black'].forEach(function(p){assert(t.includes(p),p+': '+t);});assert(!/side.cutout|open sides|long sleeves|oversized/i.test(t),t);});
    test('open front bunny '+mode,function(){var t=C.generator[mode](outfit('reverse_bunny'));['reverse bunny suit','open front bunnysuit','rabbit ear hairband','wrist cuffs','jet black','thigh-high stockings','heeled pumps','opaque satin'].forEach(function(p){assert(t.includes(p),p+': '+t);});assert((t.match(/cuffs/g)||[]).length===1,t);assert(!/pure white|reverse-cut|plain cuffs/.test(t),t);});
